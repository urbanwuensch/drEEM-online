...
if numel(files)==0
    disp(['   No ',pattern,' files found. Exiting (with an empty dataset structure)...'])
    dataout=struct;
    return
end

disp(' ')
disp(['   Reading out ',pattern,' files'])

% Clever bit to only import new samples
if not(exist("data","var"))
    data=horibaRawdata;
end
existing=data.filelist;

[~,~,ib] = intersect(existing,arrayfun(@(x) erase(x.name,pattern(2:end)),files,UniformOutput=false));

importNow=setdiff(1:height(files),ib);
files=files(importNow);

if numel(files)==0
    dataout=data;
    disp('No additional files to import.')
    return
end
...
try 
    ...
    warning('   Please manually exit Origin Pro with the Task Manager if an error occurs.')
end
...
cd(oldpath)
clearvars -except alldata workbookNames opjname sampletype workingpath data
...
nSampleExisting=dataout.nSample;
if isempty(nSampleExisting)
    nSampleExisting=0;
end
...
for j=1:numel(eemdata)
    ...
    for k=1:numel(sheetnames)
        switch sheetnames{k}
            case 'AbsSpectrumSample'
                ...
                y=y(i);
                ybackup=ybackup(i);
                    dataout.AbsI1darkSample(nSampleExisting+j,1)=worksheetData{1,3};
                try
                    dataout.Abs_horiba(nSampleExisting+j,:) = ybackup';
                    dataout.AbsI1Sample(nSampleExisting+j,:) = y';
                    dataout.Abs_wave = x;
                catch
            ...
            case 'AbsSpectrumBlank'
                ...
                x=x(i);
                y=y(i);
                dataout.AbsI1darkBlank(nSampleExisting+j,1)=worksheetData{1,3};
                try
                    dataout.AbsI1Blank(nSampleExisting+j,:) = y';
                    dataout.Abs_wave = x;
                catch
                    error(' Error, likely due to incompatible measurement settings')
                end
            case 'S1Blank'
                ...
                dataout.Ex=dataout.Abs_wave;
                dataout.Em=em;
                dataout.S1Blank(nSampleExisting+j,:,:)=flip(dat,2);
            case 'S1DarkandMcorrectBlank'
                ...
                dataout.S1DarkBlank(nSampleExisting+j,:)=[worksheetData{:,2}]';
                dataout.MCorrect(nSampleExisting+j,:)=[worksheetData{:,3}]';
            ...
            case 'R1andR1cBlank'
                ...
                dataout.R1Blank(nSampleExisting+j,:)=flip([worksheetData{:,2}]');
                dataout.AbsR1Blank(nSampleExisting+j,:)=dataout.R1Blank(nSampleExisting+j,:);
                dataout.R1DarkBlank(nSampleExisting+j,1)=[worksheetData{1,3}]';
                dataout.AbsR1darkBlank(nSampleExisting+j,1)=dataout.R1DarkBlank(nSampleExisting+j,1);
                dataout.XCorrect(nSampleExisting+j,:)=flip([worksheetData{:,4}]');
                dataout.AbsXCorrect(nSampleExisting+j,:)=dataout.XCorrect(nSampleExisting+j,:);
            ...
            case 'S1Sample'
                ...
                dat=cell2mat(worksheetData(:,2:end));
                dataout.S1Sample(nSampleExisting+j,:,:)=flip(dat,2);
            case 'S1DarkandMcorrectSample'
                ...
                dataout.S1DarkSample(nSampleExisting+j,:)=[worksheetData{:,2}]';
            ...
            case 'R1andR1cSample'
                ...
                dataout.R1Sample(nSampleExisting+j,:)=flip([worksheetData{:,2}]');
                dataout.AbsR1Sample(nSampleExisting+j,:)=dataout.R1Sample(nSampleExisting+j,:);
                dataout.R1DarkSample(nSampleExisting+j,1)=[worksheetData{1,3}]';
                dataout.AbsR1darkSample(nSampleExisting+j,1)=dataout.R1DarkSample(nSampleExisting+j,1);
            case 'Note'
                ...
                try
                    ...
                    if not(any(contains(temp,'s')))
                        dataout.integrationtime(nSampleExisting+j,:)=str2double(temp{1});
                    else
                        dataout.integrationtime(nSampleExisting+j,:)=str2double(temp{1}(1:end-2));
                    end
                catch
                end
                ...
                try
                    ...
                    if not(isempty(temp))
                        dataout.Em_parkpos(nSampleExisting+j,1)=str2double(temp{1}(1:end-3));
                    else
                        dataout.Em_parkpos(nSampleExisting+j,1)=nan;
                    end
                catch
                end
                ...
                try
                    ...
                    if not(isempty(temp))
                        dataout.Em_PixelBin(nSampleExisting+j,1)=str2double(temp{1}(strfind(temp{1},'XBin')+5));
                    else
                        dataout.Em_PixelBin(nSampleExisting+j,1)=nan;
                    end
                catch
                end
    ...
    dataout.filelist{nSampleExisting+j,1}=workbookNames_eem{j};
    dataout.opjfile{nSampleExisting+j,1}=[workingpath,filesep,opjname_eem{j}];
end
    ...
    for k=1:numel(sheetnames)
        switch sheetnames{k}
            case 'AbsSpectrumSample'
                ...
                y1=y1(i);
                ybackup=ybackup(i);
                dataout.abs.AbsI1darkSample(nSampleExisting+j,1)=worksheetData{1,3};
                dataout.abs.AbsR1darkSample(nSampleExisting+j,1)=worksheetData{1,5};
                try
                    dataout.Abs.Abs_horiba(nSampleExisting+j,:) = ybackup';
                    dataout.Abs.AbsI1Sample(nSampleExisting+j,:) = y';
                    dataout.Abs.AbsR1Sample(nSampleExisting+j,:) = y1';
                    dataout.Abs.Abs_wave = x;
                catch
                    error(' Error, likely due to incompatible measurement settings')
                end
            case 'AbsSpectrumBlank'
                ...
                y1=y1(i);
                y2=y2(i);
                dataout.Abs.AbsI1darkBlank(nSampleExisting+j,1)=worksheetData{1,3};
                dataout.Abs.AbsR1darkBlank(nSampleExisting+j,1)=worksheetData{1,5};
                try
                    dataout.Abs.AbsI1Blank(nSampleExisting+j,:) = y';
                    dataout.Abs.AbsR1Blank(nSampleExisting+j,:) = y1';
                    dataout.Abs.AbsXCorrect(nSampleExisting+j,:) = y2';
                    dataout.Abs.Abs_wave = x;
                catch
                    error(' Error, likely due to incompatible measurement settings')
                end
            case 'Note'
                ...
                try
                    ...
                    temp=erase(temp(contains(temp,'Integration Time: ')),'Integration Time: ');
                    if not(any(contains(temp,'s')))
                        dataout.Abs.integrationtime(nSampleExisting+j,:)=str2double(temp{1});
                    else
                        dataout.Abs.integrationtime(nSampleExisting+j,:)=str2double(temp{1}(1:end-2));
                    end
                catch
                end
        end
    end
    dataout.Abs.filelist{nSampleExisting+j,1}=workbookNames_abs{j};
    dataout.Abs.opjfile{nSampleExisting+j,1}=[workingpath,filesep,opjname_abs{j}];
end
dataout.nSample=size(dataout.S1Sample,1);


%% Time for a little cleanup
clearvars -except dataout path

%% Addtitional absorbance business (if needed) 
if isfield(dataout,'Abs')
   dataout.Abs.nSample=numel(dataout.Abs.filelist);
   trytoalign=true;
else
    trytoalign=false;
end
...
try
if trytoalign
    eemfl=dataout.filelist;
    absfl=dataout.Abs.filelist;
    ...
    for j=1:numel(flds)
        try %#ok<TRYNC> 
            dataout=rmfield(dataout,flds{j});
        end
        dataout.(flds{j})(ia,:)=dataout.Abs.(flds{j})(ib,:);
    end
    dataout.Abs_wave=dataout.Abs.Abs_wave;
    dataout=rmfield(dataout,'Abs');
end
catch

...
%
% I/O
% [XValResult,Model]=nway.modeleval.ncrossreg(X,y,MaxFac,Centering);
%
% INPUT
% X         : X array
% y         : y array
...
for i=1:size(SegmentsID,2)
    ...
    yc = reshape(yc,DimYc);
    [Xfactors,Yfactors,Core,B] = nway.models.npls(Xc,yc,MaxFac,NaN);
    ...
    for f=1:MaxFac
        ...
        DimXc(1)=size(Xc,1);
        Xc = reshape(Xc,DimXc);
        [ypr,T,ssx,Xres] = nway.models.npred(Xc,f,Xfactors,Yfactors,Core,B,NaN);
...
%Xc = X-ones(I,1)*Mx;
%yc = y-ones(I,1)*My;
[Xfactors,Yfactors,Core,B,ypred,ssx,ssy] = nway.models.npls(reshape(Xc,DimX),reshape(yc,DimY),MaxFac,NaN);

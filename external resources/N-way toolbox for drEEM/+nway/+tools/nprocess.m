...
% 
%     [Xnew,Means,Scales]=nway.tools.nprocess(X,Cent,Scal);
% 
%     INPUT
%     X       Data array
...
% 
%     Xnew=nway.tools.nprocess(X,Cent,Scal,mX,sX,reverse);
% 
%     INPUT
%     X       Data array
%     Cent    is binary row vector with as many elements as DimX.
...
% [Xnew,mX,sX]=nway.tools.nprocess(X,Cent,Scal,mX,sX,reverse,show,usemse);
...
% %%% I. Calculate center and scale parameters %%%
% 
%     [Xnew,Means,Scales]=nway.tools.nprocess(X,DimX,Cent,Scal);
% 
%     INPUT
%     X       Data array
%     DimX    Size of X
...
% %%% II. Use given center and scale parameters %%%
% 
%     Xnew=nway.tools.nprocess(X,DimX,Cent,Scal,mX,sX);
% 
%     INPUT
%     X       Data array
%     DimX    Size of X
...
if reverse2 == 1
   ...
   for j = ord:-1:1
      ...
      if Scal(j)
         ...
         if ~MODE
           if ~usemse
             sX{j} = (nway.math.stdnan(nway.aux.nshape(Xnew,j)')').^-1;
           else
             sX{j} = (rmsenan(nway.aux.nshape(Xnew,j)')').^-1;
           end
         end
   ...
   for j = ord:-1:1
      ...
      if Cent(j)
         ...
         if ~MODE
            if ord ~= 2
               mmm = nway.aux.nshape(Xnew,j);
               if min(size(mmm))==1
                  mmm = mmm;
               else
                  mmm = nway.math.missmean(mmm);
               end
               mX{j} = reshape(mmm,DimX(o(1:end-1)));
            else
               mX{j} = reshape(nway.math.missmean(nway.aux.nshape(Xnew,j)),DimX(o(1)),1);
            end
         end

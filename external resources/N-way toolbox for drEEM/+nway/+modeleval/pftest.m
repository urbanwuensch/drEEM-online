...
% of replicate runs
%
% [ssX,Corco,It] = nway.modeleval.pftest(NumRep,X,Fac,Options,const,OldLoad,FixMode,Weights);
%
...
if nargin==0
  ...
  disp(' ')
  disp(' I/O      : [ssX,Corco] = nway.modeleval.pftest(NumRep,X,Fac,Options,const,OldLoad,FixMode,Weights);')
  disp(' I/O short: [ssX,Corco] = nway.modeleval.pftest(NumRep,X,Fac);')
  disp(' ')
  return
end
...
for f=1:Fac
  ...
  [Factors,it,err,corcondia]=nway.models.parafac(X,f,Options,const,OldLoad,FixMode,Weights);
  ssX(f,1)=err; 
  BestErr=err;
  Corco(f,1)=corcondia;
  FactorsNow=Factors;
  ...
  for r=1:NumRep
    for f=1:Fac
      gg=plot(f+(r-1)*delta,ssX(f,r), ...
        ...
        'MarkerSize',8);
      set(gg,'userdata',[f r]);
      set(gg,'ButtonDownFcn',['fac=get(gcf,''userdata'');f=get(gco,''userdata'');figure,nway.plotting.plotfac(fac{f(1),f(2)}),set(gcf,''name'',[''Comp# '',num2str(f(1)),'' - Repl# '',num2str(f(2))]);']);
      hold on
    end
  end

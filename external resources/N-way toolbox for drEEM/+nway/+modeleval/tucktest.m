...
%
% USAGE
% [fit,label] = nway.modeleval.tucktest(X,Fac[,Options[,ConstrF,[ConstrG[,Factors[,G]]]]]);
%
% Alternatively, the output from ncrossdecomp.m may be visualized with
% tucktest using "nway.modeleval.tucktest(XvalResult);"
  ...
  for i = 1:size(possibleCombs,1)
    if prod(possibleCombs(i,:))/max(possibleCombs(i,:))>=max(possibleCombs(i,:)),
      ...
      if length(varargin)<4
        [~,~,fit(p)]=nway.models.tucker(varargin{1},label(p,:),varargin{3});
      elseif length(varargin)<5
        [~,~,fit(p)]=nway.models.tucker(varargin{1},label(p,:),varargin{3},varargin{4});
      elseif length(varargin)<6
        [~,~,fit(p)]=nway.models.tucker(varargin{1},label(p,:),varargin{3},varargin{4},varargin{5});
      elseif length(varargin)<7
        [~,~,fit(p)]=nway.models.tucker(varargin{1},label(p,:),varargin{3},varargin{4},varargin{5},varargin{6});
      else
        [~,~,fit(p)]=nway.models.tucker(varargin{1},label(p,:),varargin{3},varargin{4},varargin{5},varargin{6},varargin{7});
      end

...
%          [T,Wj,Wk]=nway.modeleval.fac2let(Xfactors);
...
%
%
% [Xfactors,Yfactors,Core,B,ypred,ssx,ssy,reg] = nway.models.npls(X,y,Fac);
% or short
% [Xfactors,Yfactors,Core,B] = nway.models.npls(X,y,Fac);
%
...
if nargin==0
   ...
   disp('  ')
   disp(' [Xfactors,Yfactors,Core,B,ypred,ssx,ssy] = nway.models.npls(X,y,Fac);')
   disp(' or short')
   disp(' [Xfactors,Yfactors,Core,B] = nway.models.npls(X,y,Fac);')
   disp(' ')
   return
elseif nargin<3
...
%    qloads = Factors(2:end);
   ...
   if ordX>1
      ...
      Core{num_lv} = nway.tuckercore.calcore(reshape(X,DimX),Xfac,[],0,1);
   else
      Core{num_lv} = 1;
   end
%   if ordY>1
%      Yfac{1}=U;Yfac(2:ordY)=Q;
%      Ycore{num_lv} = nway.tuckercore.calcore(reshape(Y,DimY),Yfac,[],0,1);
%   else
%      Ycore{num_lv} = 1;
%   end
...
elseif length(DimX)==3&~any(isnan(w_reshaped))
   ...
   wloads=nway.models.parafac(reshape(w_reshaped,DimX(2:length(DimX))),1,[0 2 0 0 NaN]');
...
%     ___________________________________________________
% 
% [Factors,it,err,corcondia,Weights] = nway.models.parafac(X,Fac,Options,const,OldLoad,FixMode,Weights);
%
...
elseif Init==10
   ...
   Opt(2) = 0;
   ERR=[];
   [Factors,it,err] = nway.models.parafac(X,Fac,Opt,const,[],[],Weights);
   ERR = [ERR;err];
   Opt(2) = 1;
   [F,it,Err] = nway.models.parafac(X,Fac,Opt,const,[],[],Weights);
   ERR=[ERR;Err];
   if Err<err
      Factors=F;
      err=Err;
...
if nargout>3      
   ...
   corcondia=nway.modeleval.corcond(reshape(x,DimX),ff,Weights,1);
end

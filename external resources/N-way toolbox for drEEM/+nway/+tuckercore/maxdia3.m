...
%
%[Gd,Od1,Od2,Od3]=nway.tuckercore.maxdia3(G,MinRot,ConvLim,Options)
%
...
%
%Upon rotation it holds that G = Od1*Gd*nway.tools.ckron(Od3',Od2')
%
%[Gd,Od1,Od2,Od3]=nway.tuckercore.maxdia3(G)
...
of0=nway.tuckercore.coredian(reshape(G,W));
...
while Rot_conv==0,
   ...
   while conv==0,
      ...
      while ~conv_a,
         i_a=i_a+1;
         dC=nway.tools.derdia3(reshape(C,W),1);
         [U D V]=svd(dC,0);
         O1=V*U';
         C=O1'*C;
         ORot1=ORot1*O1;
         
         of=nway.tuckercore.coredian(reshape(C,W));
         if show>1,
      ...
      while ~conv_b,
         ...
         dC=nway.tools.derdia3(reshape(C,W),2);
         [U D V]=svd(dC,0);
         O2=V*U';
         ORot2=ORot2*O2;
         C=C*nway.tools.ckron(I3,O2);
      ...
      while ~conv_c,
         ...
         dC=nway.tools.derdia3(reshape(C,W),3);
         [U D V]=svd(dC,0);
         O3=V*U';
         ORot3=ORot3*O3;
         C=C*nway.tools.ckron(O3,I2);
...
Gv=ORot1'*G*nway.tools.ckron(ORot3,ORot2);
format

...
%
%[Gs,Os1,Os2,Os3]=nway.tuckercore.maxswd3(G,MinRot,ConvLim,Options)
%
...
%
%Upon rotation it holds that G = Os1*Gs*nway.tools.ckron(Os3',Os2')
...
%
%[Gs,Os1,Os2,Os3]=nway.tuckercore.maxswd3(G)
...
of0=nway.tuckercore.coreswdn(reshape(G,W));
...
while Rot_conv==0,
   ...
   while conv==0,
      ...
      while ~conv_a,
         ...
         dC=nway.tools.derswd3(reshape(C,W),1);
         [U D V]=svd(dC,0);
         O1=V*U';
         C=O1'*C;
         ORot1=ORot1*O1;
         
         of=nway.tuckercore.coreswdn(reshape(C,W));
         if show>1,
      ...
      while ~conv_b,
         ...
         dC=nway.tools.derswd3(reshape(C,W),2);
         [U D V]=svd(dC,0);
         O2=V*U';
         ORot2=ORot2*O2;

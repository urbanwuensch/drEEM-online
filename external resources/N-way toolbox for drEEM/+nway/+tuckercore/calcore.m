...
%
%	
% [G]=nway.tuckercore.calcore(X,Factors,Options);
% [G]=nway.tuckercore.calcore(X,Factors);
%
...
if O, %means orthogonality
   ...
   for c=1:N,
      ...
      if MissingExist
         RedData=nway.math.missmult(kthFactor',RedData);
      else
   ...
   for c=2:N,
      ...
      RMatTmp=nway.tools.ckron(kthFactor',RMatTmp);
   end;
   
   if MissingExist
      RedData=nway.math.missmult(pinv(LMatTmp),X);
      RedData=nway.math.missmult(RedData,pinv(RMatTmp));
   else
      RedData=LMatTmp\X;

...
%
%
% nway.plotting.pfplot(X,Factors,Weights,Option);
...
% zeros else. E.g.
%
% nway.plotting.pfplot(X,factors,[],[0 0 1 0 0 0 0 1]);
%
...
if Option(3)==1
  ...
  diagonality=nway.modeleval.corcond(reshape(X,DimX),NewLoad,Weights,1);
end
model=nway.modeleval.nmodel(NewLoad);
...
if Option(4)==1
  ...
  for i=1:ord
    r=nway.aux.nshape(reshape(X-model,DimX),i)';
    varian=nway.math.stdnan(r).^2;
    subplot(aa,bb,i)
    plot(varian)
    if DimX(i)<30
      hold on

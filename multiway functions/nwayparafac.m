...
%     ___________________________________________________
%
% [Factors,it,err,corcondia] = nwayparafac(X,Fac,Options,const,OldLoad,FixMode,Weights);
%
% or skipping optional in/outputs
%
% Factors = nwayparafac(X,Fac);
%
...
if nargin==0
    ...
    disp(' ')
    disp(' Type <<doc nwayparafac>> for more info')
    disp('  ')
    disp(' [Factors,it,err,Corcondia] = nwayparafac(X,Fac,Options,const,OldLoad,FixMode,Weights);')
    disp(' or short')
    disp(' Factors = nwayparafac(X,Fac);')
    disp(' ')
...
elseif Init==10
    ...
    Opt(2) = 0;
    ERR=[];
    [Factors,~,err] = nwayparafac(reshape(X,DimX),Fac,Opt,const,[],[],Weights);
    ERR = [ERR;err];
    Opt(2) = 1;
    [F,~,Err] = nwayparafac(reshape(X,DimX),Fac,Opt,const,[],[],Weights);
    ERR=[ERR;Err];
    if Err<err
        Factors=F;
        err=Err;

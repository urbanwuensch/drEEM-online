...
close all
clearvars
load('data',filesep,'dataset1');
load('data',filesep,'dataset1res');
...
for i=1:28
   m=reshape(X(i,:),R(2),R(3));
   mn=nway.tools.setnans1(m,ExAx(1),ExAx(20),EmAx(1),EmAx(311),18,20,NaN,2,0);
   mnr=nway.tools.cmatrep(mn,'gt',999.99,NaN);
   mesh(EmAx,ExAx,mnr);
...
format compact
W=[1 3 3];
load('data\dataset3res') %[Factors,G,XExpl,Xf]=nway.models.tucker(X,R,W);
int2str(G)
figure(1);set(gcf,'Position',[-1 31 804 534]);
...
Factors = ff;
nway.plotting.plotfac(Factors,[],[1 28;250 440;250 560]);drawnow;legend('1','2','3')
...
int2str(G)
figure(1);set(gcf,'Position',[-1 31 804 534]);
nway.plotting.plotfac(Factors,[],[1 28;250 440;250 560]);legend('1','2','3')
[A,B,C]=nway.modeleval.fac2let(Factors);
...
G = reshape(G,W);
[Gv,Ov1,Ov2,Ov3]=nway.tuckercore.maxvar3(G,1e-5);
...
nway.plotting.plotfac({Ad; Bd;Cd},[],[1 28;250 440;250 560]);legend('1','2','3')

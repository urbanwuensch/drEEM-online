...
% OUTPUT
% A         Scores first mode
% H         Compressed loadings mode two
% C         Loadings mode three
% P         Compression base mode two in cell array. Loadings for e.g. slab
%           k would be Bk = P{k}*H;
% fit       Final fit (sum squared residuals)
% extra     Contains various stuff as e.g. core consistency.
%
% I/O
%
% Demo
% nway.models.parafac2('demo')
%
% Short
% [A,H,C,P]=nway.models.parafac2(X,F);
%
% Long
% [A,H,C,P,fit,extra]=nway.models.parafac2(X,F,Constraints,Options);
%
% Copyright
% Rasmus Bro
% KVL, DK, 1998
...
% $ Version 1.2 $ Date Sep 2016 $ Changed to put variance in C $ Not compiled $ RB
% $ Version 1.3 $ Apr 2019 $ Added core consistency using nway toolbox function $ RB $ Not compiled $
%
...
if nargin==0
    ...
    disp('  ')
    disp(' I/O ')
    disp(' [A,H,C,P]=nway.models.parafac2(X,F);')
    disp(' ')
    disp(' Or optionally')
    disp(' ')
    disp(' [A,H,C,P,fit]=nway.models.parafac2(X,F,Constraints,Options);')
    disp(' ')
...
if Options(4)==1
    ...
    for f = 1:F
        ...
        for s = 1:splits
            ...
            [a,h,c,p]=nway.models.parafac2(Xmiss,f,Constraints,Opt);
...
if nargin<5
    if initi==0
        ...
        Opt(4) = 0;
        Opt(5) = 1;
        [A,H,C,P,bestfit]=nway.models.parafac2(X,F,Constraints,Opt);
        AllFit = bestfit;
        for i = 2:NumRep
            Opt(3) = 2;   % Init with random
            [a,h,c,p,fit]=nway.models.parafac2(X,F,Constraints,Opt);
            AllFit = [AllFit fit];
            if fit<bestfit
                A=a;H=h;C=c;P=p;
                bestfit = fit;
    ...
    if length(Constraints)==3
        if Constraints(3)>0
            for k=1:K
                ...
                PH(PH>0)=0;
                HH = P{k}'*PH;
                H = H*(1-Constraints(3))+HH*Constraints(3);
            end
        elseif Constraints(3)<0
            for k=1:K
                PH = P{k}*H;
                PH(PH>Constraints(3))=Constraints(3);
                H = P{k}'*PH;
            end
        end
    end
...
[Consistency,G,stdG,Target]=nway.modeleval.corcond(Y,{A,H,C},[],1);
AddiOutput.CoreConsistency.Value = Consistency;
AddiOutput.CoreConsistency.Core = G;
...
while abs((fit-fitold)/fitold)>crit&it<maxit&fit>10*eps
    ...
    elseif ConstA == 1 % Nonnegativity, requires reals
        ...
        for i = 1:I
            ztz = (B'*B).*(C'*C);
            A(i,:) = nway.tools.fastnnls(ztz,Xbc(i,:)')';
        end
    ...
    if ConstB == 10 % Procrustes
        ...
        elseif ConstB == 1 % Nonnegativity, requires reals
            ...
            for j = 1:J
                ztz = (Ra'*Ra).*(C'*C);
                B(j,:) = nway.tools.fastnnls(ztz,Xac(j,:)')';
            end
    ...
    elseif ConstC == 1  % Nonnegativity, requires reals
        ...
        for k = 1:K
            ...
            C(k,:) = nway.tools.fastnnls(ztz,xab)';
        end
...
% NORMALIZE A AND B (variance in C)
if ConstB ~= 10 % Then B is eye
    for f=1:Fac,normB(f) = norm(B(:,f));end
    for f=1:Fac,normA(f) = norm(A(:,f));end
    C        = C*diag(normB)*diag(normA);
    A        = A*diag(normA.^(-1));
    B        = B*diag(normB.^(-1));
...
%	b = nway.tools.fastnnls(XtX,Xty) returns the vector b that solves X*b = y
...
%	This can be overridden with b = nway.tools.fastnnls(X,y,TOL).
%
%	[b,w] = nway.tools.fastnnls(XtX,Xty) also returns dual vector w where

...
%
% function [Factors,G,ExplX,Xm]=nway.models.tucker(X,Fac[,Options[,ConstrF,[ConstrG[,Factors[,G]]]]]);
%
...
% ---------------------------------------------------------
%
% [Factors,G,ExplX,Xm]=nway.models.tucker(X,Fac,Options,ConstrF,ConstrG,Factors,G);
% [Factors,G,ExplX,Xm]=nway.models.tucker(X,Fac);
%
% INPUT
...
SSX=nway.math.misssum(nway.math.misssum(X.^2));
...
%--------- PJG addition 4/19/22
%end
else
    ConstrG=reshape(ConstrG,size(ConstrG,1),prod(size(ConstrG))/size(ConstrG,1));
end;
%--------- PJG end 4/19/22
...
if MissingExist,
    if ~UserFactors
        [i j]=find(isnan(X));
        mnx=nway.math.missmean(X)/3;
        mny=nway.math.missmean(X')/3;
        n=size(i,1);
        for k=1:n,
            i_=i(k);
            j_=j(k);
        ...
        mnz=(nway.math.missmean(mnx)+nway.math.missmean(mny))/2;
        p=find(isnan(X));
        X(p)=mnz;
    else
        usefacinput=1;
        ...
        Fact = ff;
        Xm=nway.modeleval.nmodel(Fact,reshape(G,Fac_orig));
...
UserFactors=1;
if isempty(Factors),
    Factors=nway.aux.inituck(reshape(X,DimX),Fac_orig,2,[]);
...
if ConstrG==0,
    ...
    Fact = ff;
    G=nway.tuckercore.calcore(reshape(X,DimX),Fact,[],1,MissingExist);
    G = reshape(G,size(G,1),prod(size(G))/size(G,1));
    Core_uncon=1;
%elseif prod(size(ConstrG)==[Fac(1) prod(Fac(2:N))]),
elseif prod(size(ConstrG))== prod([Fac(1) prod(Fac(2:N))]),
    tmpM2=1;
    for k=1:N;
        if Mth(k)==4,
            tmpM1=eye(DimX(k));
        ...
        tmpM2=nway.tools.ckron(tmpM2,tmpM1);
    end
    w=ConstrG(:);
    fwz=find(w==1);
    fwnn=find(w==2);
        ...
        while ~Converged1,
            ...
            for c=1:N,
                ...
                if Mth(c)~=4,
                    ...
                    if Mth(c)==2,
                        if MissingExist,
                            [cthFactor]=nway.tools.fnipals(RedData,Fac(c),reshape(Factors(FIdx0(c):FIdx1(c)),DimX(c),Fac(c)));
                        else
                            [cthFactor]=nway.tools.fnipals(RedData,Fac(c),reshape(Factors(FIdx0(c):FIdx1(c)),DimX(c),Fac(c)));
                        end;
                    end;
                    ...
                    if Mth(c)==3,
                        if MissingExist,
                            ...
                            for i=1:2,
                                [cthFactor]=nway.tools.gsm(TempMat*cthFactor);
                            end;
                        else
                            TempMat=RedData*RedData';
                            cthFactor=reshape(Factors(FIdx0(c):FIdx1(c)),DimX(c),Fac(c));
            ...
            elseif Core_nonneg==1,
                g=nway.tuckercore.t3core(reshape(X,DimX),Fac,Factors(:),0,1);
            ...
            if (abs( keval - floor(keval) ) <=eps) | it1==1  | js==1, %Matlab 4.2 comp.
                ...
                Fact = ff;
                Xm=nway.modeleval.nmodel(Fact,reshape(G,FacNew));
                Xm = reshape(Xm,DimX(1),prod(DimX(2:end)));
            ...
            for c=1:N,
                ...
                if Mth(c)==5, %Nonneg
                    ...
                    if CalcOrdinar(c) == 1,
                        ...
                        [cthFactor,SS]=nway.tools.nonneg(X,tmpM4,tmpM3);
                ...
                if Mth(c)==8, %Unimodality
                    ...
                    if CalcOrdinar(c) == 1,
                        tmpM4=G*tmpM2;
                        if dbg, ss1=sum(sum( (X-tmpM3*tmpM4).^2 ));end;
                        cthFactor = nway.tools.unimodal(tmpM4',X');
                ...
                if Mth(c)~=4 & Mth(c)~=7
                    ...
                    if Core_const,
                        if c>1
                            cthFactor=nway.tools.normit(cthFactor);
                        end;
                    else
                        cthFactor=nway.tools.normit(cthFactor);
                    end;
                ...
                if ~Core_const
                    ...
                    if UpdateCore(c)==2,
                        G=zeros(Fac(c),prod(Fac(nway.tools.nsetdiff(1:N,c))));
                        tmpM2=1;
                        for k=[(c-1):-1:1 N:-1:c];
                            if Mth(k)==4,
                                tmpM1=eye(DimX(k));
                        ...
                        fwz=find(G_cons==1);
                        G(fwz)=tmpM2(:,fwz)\X(:);
                        G=reshape(G,Fac(c),prod(Fac(nway.tools.nsetdiff(1:N,c))));
                        CoreupdatedInner=1;
                    end;
                end;
...
%                    G_cons=reshape(G_cons(:)',newig,newjg); % PJG does not work 4/19/22
            ...
            if CoreupdatedInner==0,
                if ~Core_const & Core_cmplex==0,
                    ...
                    elseif Core_nonneg==0,
                        ...
                        Fact = ff;
                        G=nway.tuckercore.calcore(reshape(X,DimX),Fact,[],0,MissingExist);
                        G = reshape(G,size(G,1),prod(size(G))/size(G,1));
                    end
                end;
            end;

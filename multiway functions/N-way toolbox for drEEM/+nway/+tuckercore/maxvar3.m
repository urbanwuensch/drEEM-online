...
%
%[Gv,Ov1,Ov2,Ov3]=nway.tuckercore.maxvar3(G,MinRot,ConvLim,Options)
%
...
%
%Upon rotation it holds that G = Ov1*Gv*nway.tools.ckron(Ov3',Ov2')
%
%[Gv,Ov1,Ov2,Ov3]=nway.tuckercore.maxvar3(G)
...
of0=nway.tuckercore.corevarn(G);
O1=eye(W(1));
ORot1=O1;
...
while Rot_conv==0,
   ...
   while conv==0,
      ...
      while ~conv_a,
         ...
         i_a=i_a+1;
         dC=nway.tools.dervar3(reshape(C,W),1);
         [U D V]=svd(dC,0);
         O1=V*U';
         C=O1'*C;
         ORot1=ORot1*O1;
         
         of=nway.tuckercore.corevarn(C);
         if show>1,
      ...
      while ~conv_b,
         ...
         dC=nway.tools.dervar3(reshape(C,W),2);
         [U D V]=svd(dC,0);
         O2=V*U';
         ORot2=ORot2*O2;
      ...
      while ~conv_c,
         ...
         dC=nway.tools.dervar3(reshape(C,W),3);
         [U D V]=svd(dC,0);
         O3=V*U';
         ORot3=ORot3*O3;

...
% 
% I/O
% XvalResult = nway.modeleval.ncrossdecomp(Method,X,FacMin,FacMax,Segments,Cent,Show);
% 
% INPUT
...
% 
%  To visualize the output type "nway.modeleval.tucktest(XvalResult);"
...
if lower(Method(1:3)) == 'par'
   ...
   if exist('parameters')==1
      fact = nway.models.parafac(Xc,f,[1e-5 10 0 0 NaN maxit],[],parameters.fact);
   else
      fact = nway.models.parafac(Xc,f,[1e-5 10 0 0 NaN maxit]);
   end
   M = reshape(nway.modeleval.nmodel(fact),DimX(1),prod(DimX(2:end)));
   parameters.fact=fact;
elseif lower(Method(1:3)) == 'tuc'
   Xc = reshape(X- ones(I,1)*Mean',DimX);
   if exist('parameters')==1
      [fact,G] = nway.models.tucker(Xc,f,[1e-2 0 0 0 NaN maxit],[],[],parameters.fact,parameters.G);
   else
      [fact,G] = nway.models.tucker(Xc,f,[1e-2 0 0 0 NaN maxit]);
   end
   parameters.fact=fact;
   parameters.G=G;
   M = reshape(nway.modeleval.nmodel(fact,G),DimX(1),prod(DimX(2:end)))      ;
...
while ~Conv
   ...
   if Method(1:3) == 'par'
      fact = nway.models.parafac(reshape(Xcent,DimX),f,[1e-2 0 0 0 NaN maxit],[],fact);
      M = reshape(nway.modeleval.nmodel(fact),DimX(1),prod(DimX(2:end)));
   elseif Method(1:3) == 'tuc'
      [fact,G] = nway.models.tucker(reshape(Xcent,DimX),f,[1e-2 0 0 0 NaN maxit],[0 0 0],zeros(size(G)),fact,G);
      M = reshape(nway.modeleval.nmodel(fact,G),DimX(1),prod(DimX(2:end)));
   elseif Method == 'pca'
